...
Shader "Shader Weaver/dissolve"{   
	Properties {   
		...
		_Color_dummy7 ("Color dummy7", Color) = (1,1,1,1)
		...
		pass {   
			...
			*/
			...
			float4 frag (v2f i) : COLOR {
				...
				//====================================
				//============ alpha ============   
				float2  uv_alpha = i._uv_STD;
				float2 center_alpha = float2(0.5,0.5);    
				uv_alpha = uv_alpha-center_alpha;    
				uv_alpha = uv_alpha+fixed2(0,0);    
				uv_alpha = uv_alpha+fixed2(0,0.07421875)*(_Time.y);    
				uv_alpha = UV_RotateAround(fixed2(0,0),uv_alpha,0);    
				uv_alpha = uv_alpha/fixed2(1,1);    
				float2 dir_alpha = uv_alpha/length(uv_alpha);    
				uv_alpha = uv_alpha-dir_alpha*fixed2(0,0)*(_Time.y);    
				uv_alpha = UV_RotateAround(fixed2(0,0),uv_alpha,0*(_Time.y));    
				uv_alpha = uv_alpha+center_alpha;    
				float2 uv_alphaorgin = uv_alpha;
				uv_alpha = float2(uv_alpha.x >0 ?(uv_alpha.x%(1+0)) : (1+0) - abs(uv_alpha.x)%(1+0), uv_alpha.y >0 ?(uv_alpha.y%(1+0)) : (1+0) - abs(uv_alpha.y)%(1+0));
				bool discard_alpha = false;
				if(uv_alpha.x>1 || uv_alpha.y>1)
					discard_alpha = true;
				float4 rect_alpha =  float4(1,1,1,1);
				float4 color_alpha = tex2D(_wave,uv_alpha);    
				if(discard_alpha == true) color_alpha = float4(0,0,0,0);
				float aplha_alpha = 1 +-2*(_pcg) + color_alpha.r;


				//====================================
				//============ dummy7 ============   
				float2  uv_dummy7 = i._uv_STD;
				float2 center_dummy7 = float2(0.5,0.5);    
				uv_dummy7 = uv_dummy7-center_dummy7;    
				uv_dummy7 = uv_dummy7+fixed2(0,0);    
				uv_dummy7 = uv_dummy7+fixed2(0,0)*(_Time.y);    
				uv_dummy7 = UV_RotateAround(fixed2(0,0),uv_dummy7,0);    
				uv_dummy7 = uv_dummy7/fixed2(1,1);    
				float2 dir_dummy7 = uv_dummy7/length(uv_dummy7);    
				uv_dummy7 = uv_dummy7-dir_dummy7*fixed2(0,0)*(_Time.y);    
				uv_dummy7 = UV_RotateAround(fixed2(0,0),uv_dummy7,0*(_Time.y));    
				uv_dummy7 = uv_dummy7+center_dummy7;    
				float2 uv_dummy7orgin = uv_dummy7;
				uv_dummy7 = float2(uv_dummy7.x >0 ?(uv_dummy7.x%(1+0)) : (1+0) - abs(uv_dummy7.x)%(1+0), uv_dummy7.y >0 ?(uv_dummy7.y%(1+0)) : (1+0) - abs(uv_dummy7.y)%(1+0));
				bool discard_dummy7 = false;
				if(uv_dummy7.x>1 || uv_dummy7.y>1)
					discard_dummy7 = true;
				float4 rect_dummy7 =  i.rect_Sprite;
				uv_dummy7 = UV_STD2Rect(uv_dummy7,rect_dummy7);
				float4 color_dummy7 = tex2D(_MainTex,uv_dummy7);    
				color_dummy7.rgb*=color_dummy7.a;
				if(discard_dummy7 == true) color_dummy7 = float4(0,0,0,0);
				color_dummy7 = color_dummy7*_Color_dummy7;


				//====================================
				//============ mixer5 ============   
				float mixer_mixer5;
				mixer_mixer5 = clamp(aplha_alpha*1*((1)),0,1);
				mixer_mixer5 = clamp(mixer_mixer5,0,1);
				float gra_mixer5_0ListTime[2] = {0.2,0.3};
				float gra_mixer5_0ListValue[2] = {1,0};
				...
				float gra_mixer5_1ListValue[2] = {0,1};
				float gra_mixer5_1 = GradientEvaluate(gra_mixer5_1ListTime,gra_mixer5_1ListValue,2,mixer_mixer5);
				float gra_mixer5_2ListTime[2] = {0.3,1};
				float gra_mixer5_2ListValue[2] = {1,1};
				...
				result = float4(color_dummy7.rgb,color_dummy7.a*1*((1))*gra_mixer5_2);
				result = lerp(result,float4(color_color4.rgb,rootTexColor.a),clamp(color_color4.a*1*((1))*gra_mixer5_1,0,1));    
