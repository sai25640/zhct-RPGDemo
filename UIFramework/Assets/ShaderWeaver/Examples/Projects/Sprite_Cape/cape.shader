...
Shader "Shader Weaver/cape"{   
	Properties {   
		...
		_cape_mask0 ("_cape_mask0", 2D) = "white" { }
		_cape ("_cape", 2D) = "white" { }
		_capeUV ("_capeUV", 2D) = "white" { }
		...
		pass {   
			...
			*/
			...
			float4 frag (v2f i) : COLOR {
				float4 color_cape_mask0 = tex2D(_cape_mask0,i._uv_STD);    
				...
				uv_uv8 = uv_uv8+fixed2(0,-0.2)*(_Time.y);    
				...
				float4 color_uv8 = tex2D(_capeUV,uv_uv8);    
				...
				uv_image4 = uv_image4+center_image4;    
				uv_image4 = uv_image4 + uv_uv8*1*((0.2))*color_cape_mask0.r;
				float4 rect_image4 =  float4(1,1,1,1);
				float4 color_image4 = tex2D(_cape,uv_image4);    
				...
				result = lerp(result,float4(color_image4.rgb,1),clamp(color_image4.a*1*((1)),0,1));    
