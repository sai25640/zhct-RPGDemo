...
Shader "Shader Weaver/tutorial3_2"{   
	Properties {   
		...
		_tutorial3_2_mask0 ("_tutorial3_2_mask0", 2D) = "white" { }
	}   
	SubShader {   
		Tags {
			"Queue"="Transparent"
		...
		pass {   
			...
			*/
			...
			float4 Blur(sampler2D sam, float2 _uv,float2 offset,float4 rect)
			{
			    int num =12;
			...
			float4 frag (v2f i) : COLOR {
				float4 color_tutorial3_2_mask0 = tex2D(_tutorial3_2_mask0,i._uv_STD);    
				...
				uv_image2 = uv_image2+fixed2(0,1.192093E-07);    
				uv_image2 = uv_image2+fixed2(-0.1503906,2.045943E-08)*(_Time.y);    
				uv_image2 = UV_RotateAround(fixed2(0,0),uv_image2,1.371521);    
				uv_image2 = uv_image2/fixed2(1,0.9726894);    
				...
				float4 color_image2 = tex2D(_ramp,uv_image2);    
				...
				result = lerp(result,float4(color_image2.rgb,1),clamp(color_image2.a*1*((1))*color_tutorial3_2_mask0.r,0,1));    
