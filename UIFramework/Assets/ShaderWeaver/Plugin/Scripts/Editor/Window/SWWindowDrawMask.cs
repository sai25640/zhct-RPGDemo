...
namespace ShaderWeaver
{
	...
	public class SWWindowDrawMask:SWWindowLayoutV{
		...
		public void Init(SWNodeMask _node)
		{
			...
			ePopup_texRes = new SWEnumPopup(new string[]{"256x256","512x512","1024x1024"},(int)node.data.reso,SWEditorUI.MainSkin.button,
				delegate(int index){
		...
		void ShowBG_Textures()
		{
			foreach (var node in SWWindowMain.Instance.nodes) {
				...
				if (tex == null)
					continue;
//				//Provide no layerMask for masking
//				if (!node.data.layerMask.Has (node.data.id))
//					continue;
		...
		void ShowBG_Masks()
		{
			foreach (var node in SWWindowMain.Instance.nodes) {
				...
				if (!node.data.layerMask.Has (node.data.id))
					continue;
				var dt = node.data;
				SWNodeMask nodeMask = (SWNodeMask)node;
