...
//				typeof(SWEffectWindowEditMode), slotBox_left.selectSlot.name);
		...
		void ShowBG_Textures()
		{
			foreach (var node in SWWindowMain.Instance.nodes) {
				if (node.data.id == info.effector.data.id)
					continue;
				var tex = node.Texture;
				if (node is SWNodeMask)
					tex = ((SWNodeMask)node).texMask.Texture;
				if (tex == null)
					continue;
				if (!info.effector.data.layerMask.Has (node.data.id))
					continue;
...
//			Factor_CustomParamCreation ();
		...
		protected void UI_Color(string name,ref Color v,ref bool hdr,System.Action<SWBaseInfo,bool> act = null,bool before = false,bool isRightUp = false)
		{
			...
			temp = EditorGUILayout.ColorField (new GUIContent(""),v,true, true, hdr, null, GUILayout.Width (SWGlobalSettings.FieldWidth));
			if (temp != v) {
				if (before) {
					if(act!=null)
						act (info,true);
